...
class PreProcessing(object):
    ...
    def DoLastColEncoding(self, examples, last_col, choice):
        ...
        for i in range(len(examples)):
            ###########################
            if choice != 0 and choice != 3 and choice != 4:
                y[i] = y[i].strip()
                if choice == 2:
                    y[i] = y[i].strip('.')
...
def FindPerformanceMeasure(TP, TN, FP, FN,a ,b,c,d):
    print("TP = ",TP)
    print("TN = ",TN)
    print("FP = ",FP)
    print("FN = ",FN)
...
if choice == 0:
    ...
    dataset = pd.read_csv("online1_data.csv")
    last_col = ["1", "0"]
    dropping_col = []
    attrb_length = len(dataset.columns) - 1
    for i in range(attrb_length):
...
accuracy[0], tpr[0], tnr[0], precision[0], fdr[0], f1[0]= FindPerformanceMeasure(TP, TN, FP, FN,a,b,c,d)
...
for i in range(len(x_train)):
    ...
    if predictedResult == actualResult:
        ...
        not_match += 1
        # print("Does not match")
accuracy[1], tpr[1], tnr[1], precision[1], fdr[1], f1[1]= FindPerformanceMeasure(TP, TN, FP, FN,a,b,c,d)
# print("baire",match, not_match)
# accuracy_val = (match) / (match + not_match) * 100
# print("Decision Tree : ", accuracy_val, "%", "Time : ", datetime.datetime.now().time())
...
for k in k_list:
    ...
    not_match = match = 0
    for i in range(len(x_test)):
        predictedResult = decision_tree_adaboost.Prediction_Stump(weighted_majority, k_count, x_test[i])
        actualResult = y_test[i]
        if predictedResult == 1 and actualResult == 1:
            TP += 1
        elif predictedResult == -1 and actualResult == 1:
...
for k in k_list:
    ...
    not_match = match = 0
    for i in range(len(x_train)):
        predictedResult = decision_tree_adaboost.Prediction_Stump(weighted_majority, k_count, x_train[i])
        actualResult = y_train[i]
        if predictedResult == 1 and actualResult == 1:
            TP += 1
        elif predictedResult == -1 and actualResult == 1:
