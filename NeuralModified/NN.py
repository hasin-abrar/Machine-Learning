...
class NeuralNetwork:
        ...
        for i in range(1, self.N + 1):
            for r in range(1, self.L + 1):
                for j in range(1, self.K[r] + 1):
                    self.v[i][r][j] = np.matmul(np.transpose(self.w[r][j]),self.y[i][r-1] )
                    self.y[i][r][j] = self.f(self.v[i][r][j][0][0])
            val = 0.0
            for m in range(1, self.K[self.L] + 1):
                # print(i,m)
                val += (self.f(self.v[i][self.L][m]) - self.Y[i][m])\
                       *(self.f(self.v[i][self.L][m]) - self.Y[i][m])
            # print("Val : ",val)
            err.append(0.5*val)
            self.J += err[i]
            # print(err[i])
    ...
    def BackwardComputation(self):
        ...
        for i in range(1, self.N + 1):
            for j in range(1, self.K[self.L] + 1):
                # print(j)
                e[i][self.L][j] = self.f(self.v[i][self.L][j]) - self.Y[i][j]
                self.delta[i][self.L][j] = e[i][self.L][j] \
                                           * self.fDerivative(self.v[i][self.L][j])
        # for i in range(1, self.N + 1):
            for r in reversed(range(2,(self.L + 1) )):
                for j in range(1, self.K[r-1] + 1):
                    for k in range(1, self.K[r] + 1):
                        ...
                        # print(self.w[r][k])
                        e[i][r-1][j] += self.delta[i][r][k] * self.w[r][k][j]
                    # print(i,r,j)
                    self.delta[i][r - 1][j] = -1
                    self.delta[i][r-1][j] = e[i][r-1][j] \
                                            * self.fDerivative(self.v[i][r-1][j])
    ...
    def Update_Weights(self):
        ...
        for r in range(1,self.L + 1):
            for j in range(1, self.K[r] + 1):
                for i in range (1, self.N + 1):
                    for k in range(self.K[r-1] + 1):
                        delW[r][j][k] += self.delta[i][r][j] * self.y[i][r-1][k]
                for k in range(self.K[r-1] + 1):
                    delW[r][j][k] *= -self.mu
    ...
    def Predicion(self,_count,y_test,Y_actual):
        ...
        for i in range(1, _count + 1):
            ...
            for r in range(1, self.L + 1):
                v_temp[i][r] = []
                for j in range(self.K[r] + 1):
                    v_temp[i][r].append(0.0)

        for i in range(1, _count + 1):
            for r in range(1, self.L + 1):
                for j in range(1, self.K[r] + 1):
                    v_temp[i][r][j] = np.matmul(np.transpose(self.w[r][j]), y_test[i][r - 1])
                    y_test[i][r][j] = self.f(v_temp[i][r][j])
            _max = -1
            index = -1
            for j in range(1,self.K[self.L] + 1):
                if y_test[i][self.L][j] >= _max:
                    _max = y_test[i][self.L][j]
                    index = j
    ...
    def TrainData(self):
        ...
        while True:
            ...
            if self.J < 1 or _count > 4000 or (prevJ < self.J and _count > 1):
...
def initializeAll():
    ...
    df = pd.read_csv('evaluation/trainNN.txt', delimiter='\t', header=None)
...
def initializeForPrediction(L,K):
    ...
    df = pd.read_csv('evaluation/testNN.txt', delimiter='\t', header=None)
    df_array = np.array(df)
    ...
    for i in range(1, N + 1):
        y_test[i] = {}
        for r in range(L + 1):
            y_test[i][r] = np.zeros((K[r] + 1, 1))
            y_test[i][r][0] = 1.0

    for i in range(1, N + 1):
        for k in range(featureCount):
            y_test[i][0][k + 1] = x[i - 1][k]
